...
from tarp.utils.general_utils import flatten_dict, prefix_dict
...
class WandBLogger:
    ...
    def __init__(self, exp_name, project_name, entity, path, conf, exclude=None):
        ...
        filtered_config = {k: v for k, v in flat_config.items() if (k not in exclude and not inspect.isclass(v) and not isinstance(v, list) and not inspect.isfunction(v))}
        ...
        wandb.init(
            ...
            group=conf.group if 'group' in conf else None
        )
    ...
    def log_videos(self, vids, name, step=None):
        ...
        # TODO expose the FPS as a parameter
        log_dict = {name: [wandb.Video(vid, fps=40, format="mp4") for vid in vids]}
